...
class MHRHead(nn.Module):
    ...
    def __init__(
        ...
        # MPS 修正：MHR 核心模型內部使用 float64 (double)，MPS 不支援。
        # 強制將 MHR 模型加載到 CPU。
    ...
    def mhr_forward(
        ...
        # MPS 修正：MHR 模型內部包含 float64 (double) 運算，但 MPS 不支援 float64。
        # 我們將這部分運算移至 CPU 執行，然後再移回原始設備。
        orig_device = shape_params.device
        if orig_device.type == "mps":
            shape_params_cpu = shape_params.cpu()
            model_params_cpu = model_params.cpu()
            expr_params_cpu = expr_params.cpu() if expr_params is not None else None
            
            # 暫時將 mhr 模型移至 CPU (如果它還不在 CPU 上)
            self.mhr.cpu()
            ...
            # 注意：這裡不把 self.mhr 移回 MPS，因為它內部有不相容的 double 運算
        else:
            curr_skinned_verts, curr_skel_state = self.mhr(
